...
class App {
    ...
    _renderList(list) {
      ...
      <li class="item" data-id="${list.id}">
        ...
        <div class="item--container">
          <h2 class="item__title ${list.check ? "lined" : ""}">${list.content}</h2>
        </div>
    ...
    _btnDelete() {
        document.querySelectorAll(".item--delete--icon").forEach((el)=>el.addEventListener("click", this._delete.bind(this)));
    }
    _delete(e) {
    ...
    _selectAll(e) {
        ...
        btnCircleBox.querySelector(".btn--circle--check").classList.toggle("hidden");
        if (this.#selectAll) {
            document.querySelectorAll(".checked").forEach((el)=>el.classList.remove("hidden"));
            document.querySelectorAll(".item__title").forEach((el)=>el.classList.add("lined"));
            this.#list.forEach((el)=>{
                el.check = true;
            });
        } else {
            document.querySelectorAll(".checked").forEach((el)=>el.classList.add("hidden"));
            document.querySelectorAll(".item__title").forEach((el)=>el.classList.remove("lined"));
            this.#list.forEach((el)=>{
                el.check = false;
            });
        }
...
//# sourceMappingURL=index.53864b1c.js.map
